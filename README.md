...
- **Лог с уровнями** — в Serial и на страницу `/log` портала, читается и
  ...
  ```text
  [    0.043] I boot start Relay v4.3.1 built=07.08.2026
  ...
  сборки, chip ID, свободную кучу и причину сброса, а каждая перезагрузка
  объясняет свою причину. Паролей в логе нет — печатается только факт смены.
...
```
OTA headroom: 477216 of 481280 bytes used (99.2%), 4064 left
```

Это остаток до предела, за которым прошивка перестанет обновляться по воздуху.
Предел задаётся профилем платы: 481280 байт для ESP-01 и релейной платы ESP-07S
с 1 МБ флеша, 1044464 байта для D1 mini с 4 МБ. При превышении сборка падает.
...
для ESP-01/D1 mini; диагностическая сборка туда не попадает. Если версия в теге
...
GPIO2 на ESP-01 и `D4` (тот же GPIO2) на D1 mini. Банк явно требует все

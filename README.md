...
> поднимет точку доступа `relayAP`, и WiFi с MQTT нужно указать заново
...
- Нажать кнопку Save.
- Устройство сразу попробует подключиться с указанными данными, перезагрузка не нужна. При успехе статус Wi-Fi станет зелёным, появятся уровень сигнала и IP адрес, а точка доступа отключится, как только вы закроете страницу.

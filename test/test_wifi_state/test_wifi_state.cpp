...
// Точку доступа не закрываем, пока на ней кто-то сидит.
void test_ap_close_waits_while_portal_busy(void) {
  sm.begin(0, in(true));
  sm.tick(WIFI_ATTEMPT_TIMEOUT_MS, in(true));
  uint32_t t = WIFI_ATTEMPT_TIMEOUT_MS + WIFI_RETRY_STEPS_MS[0];
  sm.tick(t, in(true));
  ...
  TEST_ASSERT_EQUAL(WifiAction::None, sm.tick(t + 2000, in(true, true, true)));
  TEST_ASSERT_TRUE(sm.apUp());
  TEST_ASSERT_EQUAL(WifiAction::CloseAp, sm.tick(t + 3000, in(true, true, false)));
  TEST_ASSERT_FALSE(sm.apUp());
}
...
// Раньше этот сценарий не обрабатывался вовсе: устройство оставалось в STA
// навсегда, портал был недоступен, перенастроить можно было только прошивкой.
void test_lost_connection_returns_to_ap(void) {
  sm.begin(0, in(true));
  sm.tick(1000, in(true, true));
  TEST_ASSERT_EQUAL(WifiState::Connected, sm.state());

  TEST_ASSERT_EQUAL(WifiAction::None, sm.tick(2000, in(true, false)));
  TEST_ASSERT_EQUAL(WifiAction::OpenAp, sm.tick(2000 + WIFI_LOST_GRACE_MS, in(true, false)));
  TEST_ASSERT_EQUAL(WifiState::ApOnly, sm.state());
}

// Короткое моргание связи не должно поднимать точку доступа.
void test_short_blip_does_not_trigger_ap(void) {
  sm.begin(0, in(true));
  sm.tick(1000, in(true, true));
  ...
  TEST_ASSERT_EQUAL(WifiAction::OpenAp, sm.tick(6000 + WIFI_LOST_GRACE_MS, in(true, false)));
}
...
int main(int argc, char** argv) {
  ...
  RUN_TEST(test_ap_close_waits_while_portal_busy);
  ...
  RUN_TEST(test_lost_connection_returns_to_ap);
  RUN_TEST(test_short_blip_does_not_trigger_ap);

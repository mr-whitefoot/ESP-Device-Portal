...
//
// Всё, что знает про реле -- инверсия, сохранение состояния, суточные
// таймеры, вид в HomeAssistant -- живёт здесь. GPIO задаёт окружение платы.
// Ядро об этом не знает ничего и обращается только через device::*.

#ifndef RELAY_PIN
  #error "Не задан RELAY_PIN: выберите окружение платы с GPIO реле"
#endif

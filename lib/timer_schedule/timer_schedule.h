...
// Структура уходит в базу двоичным блобом через db[keys::timer].
// Менять раскладку нельзя, не сломав сохранённые у пользователей таймеры.
static_assert(sizeof(Timer) == 5, "раскладка Timer изменилась, сохранённые таймеры не прочитаются");
static_assert(sizeof(Timers) == 5 * TIMER_COUNT, "раскладка Timers изменилась");
